...
it does **not** touch `subscriptionPlanId`, `subscriptionStartDate`/`subscriptionEndDate`,
or `subscription_plans` — those are unrelated to passphrase/vault state. There is no
database transaction wrapping this route; the only write (`verifier`) is a single
statement, so there's no partial-write case to worry about.
