...
export default function UnlockClient({ initialUser }: { initialUser: CurrentUser }) {
  ...
  const [avatarStatus, setAvatarStatus] = useState<"loading" | "loaded" | "error">(initialUser.avatar ? "loading" : "loaded");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (res.status === 404) {
        // Session cookie is valid but the user row is gone (DB wipe / deleted
          ...
          <div className="flex flex-col items-center gap-3 mb-6">
            <div className="relative group">
              <div
                ...
                style={{
                  background: user.avatar && avatarStatus === "loaded" ? "transparent" : "var(--ui-accent-avatar)",
                  border: "2.5px solid var(--ui-avatar-border)",
                  color: "var(--ui-accent)",
                  boxShadow: "0 0 0 4px var(--ui-accent-ring)",
                }}
              >
                {user.avatar && avatarStatus !== "error" ? (
                  <>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img
                      // A cached image can finish loading before hydration attaches onLoad.
                      ref={(el) => { if (el?.complete && el.naturalWidth > 0 && avatarStatus === "loading") setAvatarStatus("loaded"); }}
                      src={user.avatar}
                      alt={user.name}
                      onLoad={() => setAvatarStatus("loaded")}
                      onError={() => setAvatarStatus("error")}
                      className={`h-full w-full object-cover ${avatarStatus === "loaded" ? "" : "hidden"}`}
                    />
