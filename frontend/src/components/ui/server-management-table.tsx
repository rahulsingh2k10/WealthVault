...
export function ServerManagementTable({
  ...
  const getCPUBars = (percentage: number, status: Server["status"]) => {
    ...
    const getBarColor = (index: number) => {
      ...
      switch (status) {
        case "active":   return "bg-foreground/60";
        case "paused":   return "bg-muted-foreground/50";
                ...
                <div className="relative grid grid-cols-12 gap-4 items-center">
                  ...
                  <div className="col-span-2 flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full overflow-hidden border border-border/30 flex items-center justify-center">
                      <div className="w-full h-full">{getCountryFlag(server.countryCode)}</div>
                    </div>
                    <span className="text-foreground">{server.serviceLocation}</span>
                  </div>
                  ...
                  <div className="col-span-2">{getCPUBars(server.cpuPercentage, server.status)}</div>
          ...
          {selectedServer && (
            ...
            >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full overflow-hidden border border-border/30 flex items-center justify-center">
                        <div className="w-full h-full scale-75">{getCountryFlag(selectedServer.countryCode)}</div>
                      </div>
              ...
              <div className="flex-1 p-4 space-y-4 overflow-y-auto">
                ...
                <div className="bg-muted/40 rounded-lg p-3 border border-border/30">
                  <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2 block">CPU Usage</label>
