/**
 * SocialLinks — floating social link rail (desktop) + expandable dock (mobile).
 * Adapted from a community snippet; shadcn tokens (hsl(var(--x)), border-border)
 * remapped to this app's --ui-* tokens, which are consumed unwrapped.
 */
"use client";
...
const PLATFORM_STYLES: Record<Platform, PlatformStyle> = {
  ...
  github: {
    label: "GitHub",
    icon: FaGithub,
    gradient: "from-zinc-800 to-zinc-600 dark:from-slate-500 dark:to-slate-200",
    hoverGradient: "from-zinc-700 to-zinc-500 dark:from-slate-400 dark:to-slate-100",
  },
  mail: {
    label: "Mail",
    icon: FaEnvelope,
  ...
  x: {
    label: "X",
    icon: FaXTwitter,
    gradient: "from-slate-900 to-slate-600 dark:from-slate-200 dark:to-slate-400",
    hoverGradient: "from-slate-800 to-slate-500 dark:from-slate-100 dark:to-slate-300",
  },
  dribbble: {
    label: "Dribbble",
    icon: FaDribbble,
...
export const SocialLinks: React.FC<SocialLinksProps> = ({
  links,
  showOnMobile = true,
  floatingButtonColor = "bg-[var(--ui-subtle-bg)]",
}) => {
              ...
              >
                <a
                  ...
                  className="flex items-center justify-between w-44 h-14 px-4 ml-[-120px]
                             ...
                             rounded-r-xl relative overflow-hidden border border-[var(--ui-card-border)]
                             bg-[var(--ui-card-bg)] shadow-md hover:shadow-lg"
                >
                  {/* Gradient Overlay */}
                  <div
          ...
          <div className="relative">
                  ...
                  >
                    <div
                      className={`w-14 h-14 rounded-full bg-gradient-to-br ${style.gradient}
                                 ...
                                 transition-transform duration-300 border border-[var(--ui-card-border)]`}
                    >
                    ...
                    <div className="absolute top-1/2 -translate-y-1/2 right-16
                                    bg-[var(--ui-modal-bg)] text-[var(--ui-text-pri)]
                                    text-xs font-medium px-3 py-1.5 rounded-md shadow-md
                                    opacity-0 group-hover:opacity-100 transition-opacity">
                      {style.label}
                      <div className="absolute top-1/2 -translate-y-1/2 -right-1 w-2 h-2 bg-[var(--ui-modal-bg)] rotate-45" />
                    </div>
                  </a>
                );
              })}
            ...
            <button
              ...
              className={`relative flex items-center justify-center w-16 h-16 rounded-full shadow-2xl active:scale-95
                         transition-all duration-300 border border-[var(--ui-card-border)] overflow-hidden ${floatingButtonColor}`}
            ...
            >
              ...
              <div className="absolute inset-0 bg-[var(--ui-subtle-bg)]" />
            </button>
          </div>
        </div>
      )}
