...
- **`fullName`** — Sourced from the OAuth provider's profile. Refreshed on every login for Google, X, and LinkedIn. For Apple, the provider sends a name only on the very first authorization — every later Apple login has no name to read and writes the fallback `"Apple User"` over whatever name was stored, since the callback's own fallback logic runs again with nothing to fall back from.
...
- **`avatar`** — Either the OAuth provider's profile picture URL, or a user-uploaded base64 JPEG data URL (set via `PATCH /api/auth/avatar`). `NULL` for Apple sign-ins that never uploaded a photo. On every Google/LinkedIn login the callback's `upsert` sets `avatar` to the provider's current picture URL whenever it returns one (which it almost always does), and on every X login it is set unconditionally (`profile_image_url`, or a `unavatar.io` fallback if X gives none) — so a custom-uploaded photo on those three providers is overwritten the next time the user signs in through the provider. The Apple callback never writes to this column at all (Apple's OAuth response carries no photo), so a custom upload on an Apple account is never overwritten by sign-in.
