...
- **`subscriptions.cancel(id, true)` (cancel-at-cycle-end) appears to be a no-op on Razorpay for a mid-term card-mandate subscription** (found during the 2026-08-31/09-01 manual test pass): called it twice on a card subscription in its first cycle (`paid_count: 1`) — the subscription stayed `status: active`, `charge_at` unchanged, `end_at` unchanged, `has_scheduled_changes: false`, no pending update. Whether Razorpay's scheduler actually stops the renewal at `current_end` in this state is **unverified** (couldn't be tested without a subscription reaching cycle end, and test mode may not simulate it). This affects the **plain "Cancel subscription"** flow too: the app drops the user to FREE at `currentEnd` based on local state, but if Razorpay keeps charging the card, that's a real billing bug. **Verify in live mode** (or with a short-interval test plan) before go-live: confirm a `cancel_at_cycle_end` subscription actually fires `subscription.halted`/`subscription.cancelled` at cycle end and stops charging. If it doesn't reliably, switch the plain-cancel flow to also schedule an immediate `cancelNow` at `currentEnd` (a small cron / lazy check) rather than trusting Razorpay's deferred cancel.
