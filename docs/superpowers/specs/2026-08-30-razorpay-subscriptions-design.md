...
- **Seamless in-place plan change for UPI AutoPay subscribers** (found during the 2026-08-31 manual test pass): Razorpay's `subscriptions.update({ plan_id, schedule_change_at: "cycle_end" })` is rejected for card mandates — `"Only offers can be updated for subscriptions when payment mode is domestic card."` — so change-plan must cancel the old subscription and create a new one (new mandate, new checkout, ₹5 refundable auth). UPI AutoPay mandates DO support `subscriptions.update`. Follow-up: read the mandate's payment method from the `subscription.charged` webhook / payment entity and, for UPI subscribers, do the plan change in place via `subscriptions.update` (no new checkout); keep cancel-and-recreate as the card fallback. Also consider defaulting Checkout to UPI AutoPay.
- ~~**Cancel a scheduled plan change.**~~ **DONE (2026-09-01, `aaff1b9`).** Reworked so it's safe: `/verify` of the superseding row no longer touches the old subscription — `applySubscriptionEvent` cancels the old one (immediately, `cancelNow`) only when the new plan's `subscription.activated` webhook lands. Until then the old plan is untouched, so `POST /api/subscription/cancel-scheduled-change` just cancels the not-yet-active new subscription and deletes the pending row; the user stays on their current plan with nothing to reverse. Manage screen has a "Cancel scheduled change" button while one is pending.
...
- ~~**Pre-existing flaky e2e**~~ **FIXED (2026-09-01, `da24704`).** `tests/e2e/unlock-authenticated.spec.ts` › "uploading an avatar…" was flaky (~2 of 3 runs) on `main` and this branch — a race in `frontend/src/app/unlock/page.tsx`'s `handleAvatarChange`: `setUser((prev) => prev ? { ...prev, avatar } : prev)` silently dropped the upload if the initial `GET /api/auth/me` was still in flight (a real user uploading within ~200 ms of page load hit it too). Now stashes the upload in a ref and the load handler applies it. Fixed here (not strictly in scope) so `./run-tests.sh` is fully green before the merge.
- **`subscriptions.cancel(id, true)` (cancel-at-cycle-end) appeared to be a no-op on Razorpay for a mid-term card-mandate subscription** (found 2026-08-31/09-01): called it twice on a first-cycle card subscription — stayed `status: active`, `charge_at`/`end_at` unchanged, no pending update. Reworked the cancel flow to not depend on it (below).
- **Plain-cancel = pause the Razorpay subscription** (`56a623c` then reworked 2026-09-01): `POST /api/subscription/cancel` calls `provider.pauseSubscription` (Razorpay `subscriptions.pause(id, {pause_at:"now"})` — sets `charge_at: null`, so the pending renewal never fires) and flags the row locally. `grants()` now treats a `cancelAtCycleEnd` row as granting **only** while `now < currentEnd`, so access falls to FREE at the paid-through date. `POST /api/subscription/resume` calls `provider.resumeSubscription` (`subscriptions.resume(id, {resume_at:"now"})`) and clears the flag — fully reversible, no lost cycle, no re-mandate. Resume is allowed only while `currentEnd` is still in the future (409 otherwise → re-subscribe). `applySubscriptionEvent` keeps a defence-in-depth `cancelNow` if a `subscription.charged` somehow lands on a flagged row. **Before go-live, verify once** that `subscriptions.pause` succeeds on an *active* card-mandate subscription (couldn't reach `active` in test mode without a real checkout; docs list pause/resume as first-class ops, not `update`-gated). **Housekeeping (not urgent):** a subscription the user cancels and never resumes stays `paused` on Razorpay forever (harmless — no charges); a periodic `cancelNow` sweep of long-paused subs would tidy that up.
