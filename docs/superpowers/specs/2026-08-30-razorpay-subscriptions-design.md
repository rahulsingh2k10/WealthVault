...
- **Plain-cancel is now local + webhook-committed** (`<commit>`, 2026-09-01): `POST /api/subscription/cancel` sets only the local `cancelAtCycleEnd` flag (which drives display + the go-to-FREE-at-cycle-end path) and does **not** call Razorpay. `POST /api/subscription/resume` flips it back — instant, nothing to reverse (mirrors cancel-scheduled-change). `applySubscriptionEvent` calls `provider.cancelNow` + marks the row `cancelled` (access frozen at the pre-charge `currentEnd`) when a `subscription.charged` renewal lands on a still-flagged row. **Known gap for go-live:** if the user cancels and never resumes, Razorpay charges one more renewal cycle before that webhook fires — the app freezes access correctly but the user is out one cycle's money. A cycle-end cron (run `cancelNow` on rows where `cancelAtCycleEnd && currentEnd` is within ~24h) closes this; `provider.fetchSubscription` + a scheduled task, no new infra beyond a cron trigger.
