...
- **Cancel a scheduled plan change.** Once a change-plan checkout completes, the superseding subscription is scheduled and the current one is set to cancel at cycle end; there's no in-app way to undo that (would need to cancel the new Razorpay subscription immediately and reverse the old one's `cancel_at_cycle_end`, whose reversibility on Razorpay is unconfirmed). For now the Manage screen just blocks scheduling a second change while one is pending. Follow-up: a "keep my current plan" action, once the Razorpay un-cancel path is verified.
